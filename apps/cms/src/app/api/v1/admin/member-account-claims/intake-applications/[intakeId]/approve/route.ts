...
import { reviewIntakeSchema } from '@/modules/recruitment/member-account-claim/schemas'
import { approveMemberIntake } from '@/modules/recruitment/member-account-claim/use-cases/review-intake'
...
export async function POST(
  ...
  try {
    ...
    const command = reviewIntakeSchema.parse(await request.json())
    const payload = await getPayload({ config })
    const req = await createPayloadRequest(payload, request.headers)
    await approveMemberIntake(payload, req, intakeId, {

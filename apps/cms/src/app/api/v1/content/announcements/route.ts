...
} from '@/modules/content/announcements/public-read'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = publicAnnouncementQuerySchema.parse(
      Object.fromEntries(searchParams.entries()),
    )
    const payload = await getPayload({ config })
    const req = await createPayloadRequest(payload, request.headers)
    const result = await listPublicAnnouncements(payload, req, query)
